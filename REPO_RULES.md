# Repository rules — Repo Governance Templates
...
This repository is the public source kit. Root `*.template.*` files and `github/` are reusable
inputs; bootstrap and update scripts install them into other repositories; `tests/` exercises
the kit only in disposable repositories; `docs/` explains architecture, migration, security,
and platform limits. Private infrastructure configuration and operational recovery material are
