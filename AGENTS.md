# Working agreement — Repo Governance Templates
...
This repository publishes the reusable Repo Governance V3 templates, bootstrap/update tools,
and disposable acceptance suites. Keep the public kit agent-neutral and free of private
infrastructure details; project installations are generated from the reviewed templates.

- Install local hooks: `lefthook install`
- Bootstrap a fixture: `./new-governed-repo.sh --target /tmp/governed-fixture --name Fixture --owner test-owner`
- Test on Linux: `sh tests/acceptance.sh`
- Test PowerShell compatibility: `pwsh -NoLogo -NoProfile -File tests/acceptance.ps1`
- Parse shell entry points: `sh -n ./*.sh ./*.template.sh scripts/ci/*.sh`
...
- Keep reusable templates free of organization-, person-, machine-, network-, and
  environment-specific operational data.
- When a manifest-owned template changes, update its `governance-manifest.json` SHA-256 and
  preserve any still-supported prior shipped hash in `knownHashes`.
- Keep POSIX and PowerShell behavior aligned; explicitly disclose when a path was not executed
  on native Windows.
- Remote-policy commands remain plan-only unless the human owner explicitly authorizes apply.
